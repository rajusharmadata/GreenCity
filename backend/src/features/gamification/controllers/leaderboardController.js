...
const TIERS = [
  ...
  { name: 'Planet Saviour', min: 3000, max: Infinity, icon: '🏆', color: '#f59e0b' }
];
...
const computeBadges = (points = 0) => BADGES.filter((b) => points >= b.points).map((b) => b.name);

// ─────────────────────────────────────────────────────────────────────────────
// addPoints — pure utility, called from other controllers (not an HTTP handler,
// so it stays a plain async function rather than asyncHandler).
// Only touches the single user's leaderboard doc — O(1) DB writes per call,
// not O(n). Full rank recompute happens separately via updateLeaderboard().
// ─────────────────────────────────────────────────────────────────────────────
export const addPoints = async (userId, points, reason = 'Action') => {
  try {
    if (!userId || points === undefined || points === null) return null;

    const user = await User.findById(userId).select('points tier name avatar reportsCount badges');
    if (!user) return null;
    ...
    await Promise.all([
      ...
      PointHistory.create({ userId, delta: points, reason, totalAfter: newPoints }).catch(() => {}),
    ...
    // Never crash the parent request over a non-critical side effect.
    console.error('addPoints error (non-fatal):', err.message);
    return null;
  }
};

// ─────────────────────────────────────────────────────────────────────────────
// updateLeaderboard — call from a scheduled cron job ONLY, not per-request.
    ...
    const bulkOps = users.map((user, idx) => ({
      updateOne: {
        ...
        update: {
          ...
          $setOnInsert: { previousRank: null, rankChange: 0 }
        },
        upsert: true
      }
    }));
...
export const getLeaderboard = asyncHandler(async (req, res) => {
  ...
  const [leaderboard, total] = await Promise.all([
    Leaderboard.find(filter).sort({ points: -1 }).skip(skip).limit(limit).lean(),
...
export const getUserRank = asyncHandler(async (req, res) => {
  ...
  if (!userEntry) throw new ApiError(404, 'User not found in leaderboard');

  const nearbyRankings = await Leaderboard.find({
    points: { $gte: Math.max(0, userEntry.points - 200), $lte: userEntry.points + 200 }
  })
    .sort({ points: -1 })
    .limit(11)
    .lean();

  res.json({ success: true, data: { user: userEntry, nearbyRankings } });
});
...
export const getLeaderboardStats = asyncHandler(async (req, res) => {
  const [totalUsers, pointsAgg, tierDist, topContributors] = await Promise.all([
    User.countDocuments(),
